...
def _normalize_mixed_path(path_str: str) -> str:
    """
    Normalize a possibly mixed Windows/Linux path string.
    ...
    Relative paths are resolved against repository ROOT so the script does not
    ...
    # Normalize both separators into the local OS separator first.
    s = s.replace("\\", os.sep).replace("/", os.sep)
    s = os.path.normpath(s)

    # Resolve relative CSV paths against repo root.
    if not os.path.isabs(s):
        s = os.path.normpath(os.path.join(ROOT, s))
...
def process_one(task):
    ...
    try:
        scene_path = _normalize_mixed_path(row["pcd_path"])
        ...
        if G_USE_BOP_GT:
            depth_path = _normalize_mixed_path(row["depth_path"])
...
def _method_groups(config_dir: str):
    groups = [
        ...
        ("ours_no_pose_pipeline", os.path.join(config_dir, "ablation_ours_no_pose_pipeline.yaml")),
...
def main():
    ...
    ap.add_argument("--config_dir", type=str, default="configs",
                    help="Directory containing the 5 main ablation YAML files.")
    ...
    ap.add_argument("--groups", type=str, default="",
                    help="Optional comma-separated subset of groups to run. "
                         "Choices: baseline,ours_no_rsmrq,ours_no_robustvote,ours_no_pose_pipeline,ours")
    args = ap.parse_args()
